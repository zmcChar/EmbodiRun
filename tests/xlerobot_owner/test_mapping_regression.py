...
def test_limited_backward_translation_uses_shared_cartesian_fraction() -> None:
    ...
    assert (
        result[_left("shoulder_lift")]
        + result[_left("elbow_flex")]
        + result[_left("wrist_flex")]
...
def test_trigger_closes_release_holds_and_selected_direction_opens_at_rate() -> None:
    ...
    for seq in range(7, 11):
        opened = mapper.map(_frame(seq, left_grip=True, left_trigger=1,
                                   left_direction="open"), state, 0.05)
        expected += 25.0 * 0.05
...
def test_direction_toggle_with_held_trigger_needs_release_and_resqueeze():
    ...
    for seq in (2, 3):
        switched = mapper.map(_frame(seq, left_grip=True, left_trigger=1,
                                    left_direction="open"), state, 0.05)
    ...
    opened = mapper.map(_frame(5, left_grip=True, left_trigger=0.5,
                              left_direction="open"), state, 0.05)
...
def test_left_and_right_claw_direction_are_independent():
    ...
    initial = replace(initial, controllers={
        **initial.controllers,
        "right": replace(initial.controllers["right"], gripper_direction="open"),
    })
    mapper.map(initial, state, 0.05)
    squeezed = replace(initial, seq=1, timestamp_ms=50, controllers={
        side: replace(c, trigger=1) for side, c in initial.controllers.items()
    })
...
def test_real_pose_cartesian_step_keeps_direction_limits_and_has_no_queued_motion(direction):
    ...
    mapper = QuestMapper()
    mapper.map(_frame(0, left_grip=True), state, .05, limits)
    position = tuple(a + b for a, b in zip((0, 1, -.5), direction))
    action = mapper.map(_frame(1, left_grip=True, left_position=position), state, .05, limits)
    for name in (_left(j) for j in ARM_JOINTS):
        assert abs(action[name] - state[name]) <= .750001
        if name in limits:
    ...
    for seq in range(2, 22):
        held = mapper.map(_frame(seq, left_grip=True, left_position=position), state, .05, limits)
...
def test_shoulder_limit_cannot_freeze_independent_pan(depth_noise):
    ...
    mapper = QuestMapper()
    mapper.map(_frame(0, left_grip=True), state, .05, limits)
    frame = _frame(1, left_grip=True, left_position=(.005, 1, -.5 + depth_noise))
    result = mapper.map(frame, state, .05, limits)
    assert result[_left("shoulder_pan")] == pytest.approx(.6)
    ...
    for seq in range(2, 22):
        held = mapper.map(replace(frame, seq=seq), state, .05, limits)
...
def test_wrist_limit_does_not_freeze_pan_or_planar_translation():
    ...
    mapper = QuestMapper()
    mapper.map(_frame(0, left_grip=True), state, .05, limits)
    frame = _frame(1, left_grip=True, left_position=(.005, 1.002, -.5))
    result = mapper.map(frame, state, .05, limits)
    assert result[_left("shoulder_pan")] == pytest.approx(.6)
    ...
    assert new[1] > old[1] + .0005
    assert _left("wrist_flex") in mapper.diagnostics["left"]["joint_limit_joints"]
    for joint in ARM_JOINTS:
        assert abs(result[_left(joint)] - state[_left(joint)]) <= .750001
...
def test_wrist_rotation_at_limit_does_not_freeze_other_axes():
    ...
    mapper = QuestMapper()
    mapper.map(_frame(0, left_grip=True), state, .05, limits)
    frame = _frame(1, left_grip=True, left_position=(.005, 1, -.5))
...
def test_deadband_accumulates_slow_height_and_wrist_while_pan_is_moving():
    state = _state()
    mapper = QuestMapper()
    mapper.map(_frame(0, left_grip=True), state, .05)
    for seq in range(1, 7):
        frame = _frame(seq, left_grip=True, left_position=(seq * .005, 1 + seq * .0004, -.5))
        angle = math.radians(seq * .2) / 2
        left = replace(frame.controllers["left"], orientation=(math.sin(angle), 0, 0, math.cos(angle)))
        frame = replace(frame, controllers={**frame.controllers, "left": left})
        result = mapper.map(frame, state, .05)
