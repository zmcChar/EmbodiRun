...
def clocked(robot, bus, monkeypatch, *, until=.65, alternating=False):
    ...
    def sleep(delay):
        clock.t += max(delay, .000001)

    monkeypatch.setattr(hardware, "time", SimpleNamespace(
        monotonic=monotonic, sleep=sleep, time_ns=lambda: int(clock.t * 1e9)))
    ...
    def read(port, motor_id, address, length):
        if address == 56:
            ...
            moving = (int(clock.t / .02) % 2) if alternating else int(clock.t < until)
...
@pytest.mark.parametrize("reason,write,other,expected", [
    ("operator", True, False, True), ("operator_retry", True, False, True),
    ("watchdog", True, False, False), ("close", True, False, False),
    ("verified held restart", False, False, False), ("operator", True, True, False),
])
def test_single_packet_with_late_zeros_only_extends_explicit_idle_stop(tmp_path, monkeypatch, reason, write, other, expected):
    ...
    try:
        ...
        assert (.65 < clock.t < .75) if expected else (.4 <= clock.t < .41)
        assert len(packet.calls) == 2 * len(result["samples"])
        assert ("arms" in robot._active_scopes) is other
        if expected:
            assert all(not m["Present_Velocity"] and not m["Moving"]
                       for s in result["samples"][-2:] for m in s["motors"].values())
    finally:
...
def test_extended_window_still_rejects_and_errors_do_not_extend(tmp_path, monkeypatch, mode):
    ...
    try:
        ...
        assert (.4 <= clock.t < .41) if mode == "write_error" else (1.5 <= clock.t < 1.51)
    finally:
        robot.close()


@pytest.mark.parametrize("bad_first", [False, True])
def test_fault_resume_plus_burst_plus_extended_window_requires_valid_consecutive_zeros(tmp_path, monkeypatch, bad_first):
    ...
    try:
        ...
        assert result["stationary_confirmed"] and .65 < clock.t < .75
