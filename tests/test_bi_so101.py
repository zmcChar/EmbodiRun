...
@pytest.fixture
def robot():
    config = BiSO101Config.from_mapping("pair", {
        "left_port": "/dev/left", "right_port": "/dev/right",
        "max_joint_step_deg": 5.0, "max_gripper_step": 10.0,
    })
    left = Bus([0, 1, 2, 3, 4, 50])
    right = Bus([5, 6, 7, 8, 9, 60])
    adapter = BiSO101Adapter(
        config, left=SO101Adapter(config.left, controller=left),
...
def result(rows, names=BI_SO101_POSITION_FEATURES):
    return PolicyResult(
        request_id="request", session_id="session", step_id=0, session_revision=1,
...
def test_explicit_clip_limits_both_arms(robot):
    ...
    for side in ("left", "right"):
        action.values[side] = {"joint_positions_deg": [100] * 5, "gripper_position": 100}
    adapter.execute(action)
...
@pytest.mark.parametrize("options", [
    {"right_port": "/dev/left"}, {"right_port": ""},
    {"left_calibration_id": "same", "right_calibration_id": "same"},
    {"port": "/dev/other"}, {"calibration_id": "same"}, {"max_joint_step_deg": -1},
    {"unknown": True},
])
...
@pytest.mark.parametrize("names,row", [
...
def test_control_runtime_sends_12_state_values_and_three_images(robot):
    ...
    class Client:
        def open_session(self, **kwargs):
            assert kwargs == {"robot_id": "pair", "action_space": "pi05.action_chunk.v1"}
            return Session("session", 0)

        def step(self, request):
            assert list(request.state.values()) == [0, 1, 2, 3, 4, 50, 5, 6, 7, 8, 9, 60]
            assert [image.name for image in request.images] == ["front", "left_wrist", "right_wrist"]
            return result([[1, 2, 3, 4, 5, 55, 6, 7, 8, 9, 10, 65]] * 2)

    runtime = ControlRuntime(adapter, Client(), instruction="pick chips", mapper=Pi05BiSO101Mapper(),
                             chunk_steps=2, control_hz=15, sleep=lambda _: None)
