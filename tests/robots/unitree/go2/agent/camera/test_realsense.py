...
from embodied_runtime.robots.unitree.go2.agent.camera.depth_store import DepthStore
from embodied_runtime.robots.unitree.go2.agent.camera.frame_store import FrameStore
from embodied_runtime.robots.unitree.go2.agent.camera.realsense import RealSenseWorker
from embodied_runtime.robots.unitree.go2.agent.camera.server import CameraHTTPServer
from embodied_runtime.robots.unitree.go2.agent.camera.types import CameraStreamError, DepthAnalysis
...
def test_single_pipeline_aligns_and_publishes_one_atomic_rgbd_pair() -> None:
    ...
    try:
        metadata = server.observations.depth_payload()
        assert metadata["navigation_ready"] is True
        assert metadata["pair_sequence_matched"] is True
        observation = server.observations.observation_payload()
...
def test_pyrealsense_is_loaded_only_when_backend_runs() -> None:
    ...
    with (
        mock.patch(
            "embodied_runtime.robots.unitree.go2.agent.camera.realsense.importlib.import_module",
