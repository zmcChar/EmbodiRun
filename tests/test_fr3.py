...
def test_joint_motion_and_observation_follow_franky_contract() -> None:
    ...
    adapter.execute(
        action({"type": "joint_position", "joint_positions_rad": [0.1] * 7})
    )
...
def test_joint_step_limit_is_fail_closed() -> None:
    ...
    with pytest.raises(FR3AdapterError, match="exceeds"):
        adapter.execute(
            action({"type": "joint_position", "joint_positions_rad": [0.5] * 7})
        )
...
def test_relative_cartesian_and_gripper_commands() -> None:
    ...
    adapter.connect()
    adapter.execute(
        action({"type": "cartesian_delta", "translation_m": [0.01, 0.0, 0.0]})
    )
