...
from embodied_runtime.deployment.unitree.go2 import Go2AgentDeployer as PublicGo2AgentDeployer
from embodied_runtime.deployment.unitree.go2.deployer import Go2AgentDeployer
from embodied_runtime.deployment.unitree.go2.options import (
    CameraStartOptions,
    ControlStartOptions,
    StartOptions,
)
from embodied_runtime.deployment.unitree.go2.services import SERVICE_MODULES
from embodied_runtime.deployment.unitree.go2.transport import CommandResult

from .fakes import FakeTransport


def test_public_deployer_identity_and_service_modules_are_canonical() -> None:
    assert PublicGo2AgentDeployer is Go2AgentDeployer
...
def _package_tree(root: Path) -> Path:
    ...
    (package / "robots/unitree/go2/agent/control").mkdir(parents=True)
    (package / "robots/unitree/go2/agent/camera").mkdir(parents=True)
    (package / "__init__.py").write_text("", encoding="utf-8")
    (package / "robots/unitree/go2/agent/control/__main__.py").write_text(
        "raise SystemExit(0)\n", encoding="utf-8"
    )
    (package / "robots/unitree/go2/agent/camera/__main__.py").write_text(
        "raise SystemExit(0)\n", encoding="utf-8"
    )
    cache = package / "robots/unitree/go2/agent/control/__pycache__"
    cache.mkdir()
...
def test_install_uploads_repository_package_and_manifest(tmp_path: Path) -> None:
    ...
    assert transport.files[f"{prefix}/robots/unitree/go2/agent/control/__main__.py"]
...
def test_status_and_stop_verify_service_module_before_signalling() -> None:
    ...
    module = "embodied_runtime.robots.unitree.go2.agent.control"
