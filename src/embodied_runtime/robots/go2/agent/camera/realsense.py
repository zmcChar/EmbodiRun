...
from .types import CameraStreamError, DepthAnalysis, VideoProfile
...
class RealSenseWorker:
    ...
    def _validated_profile(
        ...
        try:
            ...
            raise CameraStreamError(
                f"could not inspect active RealSense profile: {error}"
            ) from error
        mismatches = []
        if width != self.width:
    ...
    def _device_name(self, rs: Any, pipeline_profile: Any) -> str:
        if self.serial:
            return f"realsense:{self.serial}"
        try:
            serial = pipeline_profile.get_device().get_info(rs.camera_info.serial_number)
    ...
    def _run_once(self, rs: Any) -> None:
        ...
        try:
            ...
            color_profile = self._validated_profile(
                pipeline_profile,
                rs.stream.color,
                rs.format.bgr8,
                "BGR8",
            ...
            depth_profile = self._validated_profile(
                pipeline_profile,
                rs.stream.depth,
                rs.format.z16,
                "Z16",
            ...
            if actual_depth_scale <= 0 or not math.isclose(
                ...
                raise CameraStreamError(
                    f"RealSense depth scale {actual_depth_scale} does not match confirmed {self.expected_depth_scale}"
                )

            align = rs.align(rs.stream.color)
            device_name = self._device_name(rs, pipeline_profile)
            ...
            while not self._stop.is_set():
                ...
                try:
                    ...
                    raise CameraStreamError(
                        f"could not read RealSense source identity: {error}"
                    ) from error
                if (
                    last_source_frame_number is not None
                    and source_frame_number <= last_source_frame_number
                ):
                    raise CameraStreamError("RealSense source frame number did not increase")
                if (
                    last_source_timestamp_ms is not None
                    and source_timestamp_ms <= last_source_timestamp_ms
                ):
                    raise CameraStreamError("RealSense source timestamp did not increase")

                color_raw = self._validated_frame_bytes(
                    color_frame,
                    color_profile,
                    "color",
                )
                depth_raw = self._validated_frame_bytes(
                    depth_frame,
                    depth_profile,
                    "aligned depth",
                )
