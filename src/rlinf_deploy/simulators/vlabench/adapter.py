...
runtime boundary.  Importing :mod:`rlinf_deploy.simulators` therefore does
...
class VLABenchAdapter(SimulatorAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        self._environment_factory = (
            environment_factory or make_semantic_vlabench_environment
        )
        self._clock = clock
    ...
    def instruction(self) -> str:
        if self._instruction is None:
            raise RuntimeError(
                "VLABench adapter must be reset before reading its instruction"
            )
    ...
    def reset(
        ...
        *,
        ...
        if task is not None and task.strip() != self.config.task:
            raise ValueError(
                f"adapter is bound to VLABench task {self.config.task!r}, "
                f"not {task!r}"
            )
    ...
    def _seed_process(self, seed: int | None) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                "VLABench execution requires NumPy in the isolated simulator "
                "environment"
            ) from error
        np.random.seed(seed)
...
def _encode_frame(name: str, value: Any) -> CameraFrame:
    try:
        ...
        raise RuntimeError(
            "VLABench image encoding requires NumPy and OpenCV"
        ) from error
...
def _action_array(values: Any) -> Any:
    ...
    if action.shape != (VLABENCH_ACTION_DIM,):
        raise ValueError(
            f"VLABench action must have shape ({VLABENCH_ACTION_DIM},), "
            f"got {action.shape}"
        )
