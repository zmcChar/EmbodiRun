...
class Go2AgentDeployer:
    ...
    def install(self, package_root: Path | None = None) -> dict[str, object]:
        ...
        remote_package = posixpath.join(self.source_root, "rlinf_deploy")
...
def _validate_package_root(path: Path) -> None:
    if path.name != "rlinf_deploy" or not (path / "__init__.py").is_file():
        raise ValueError(f"package root is not an rlinf_deploy source tree: {path}")
