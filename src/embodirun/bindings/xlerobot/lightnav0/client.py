...
class LightNav0Client:
    ...
    async def start(self) -> None:
        ...
        if (
            ...
            or adapter.get("action_space") != ACTION_SPACE
        ):
        ...
        with self._lock:
            ...
            if previous is None:
                session = self._client.open_session(
                    robot_id=self._robot_id, action_space=ACTION_SPACE
                )
            else:
    ...
    async def predict(
        self, rgb: Any, *, instruction: str, session_id: str, timestamp_s: float
    ) -> WaypointPrediction:
    ...
    def _predict(
        ...
        with self._lock:
            ...
            if (
                result.session_id != remote_id
                or result.request_id != request.request_id
                or result.step_id != step
            ):
                raise ValueError(
                    "inference response does not match the current request"
                )
            if (
                result.action_space != ACTION_SPACE
                or len(result.actions) != 1
                or result.actions[0].kind != "waypoints"
            ):
                raise ValueError(
                    "inference response is not a LightNav-0 waypoint chunk"
                )
            ...
            if (
                ...
                or len(rows) != 10
                or type(metadata.get("stop")) is not bool
            ):
                raise ValueError(
                    "LightNav-0 response requires ten waypoints and a boolean stop"
                )
            if any(
            ...
            return WaypointPrediction(
                WaypointChunk(
                    tuple(tuple(float(v) for v in row) for row in rows), metadata
                )
            )
    ...
    def _close(self) -> None:
        ...
        if errors:
            raise RuntimeError(
                "failed to close one or more LightNav-0 sessions"
            ) from errors[0]
...
def create_client(args: argparse.Namespace) -> LightNav0Client:
    ...
    if args.inference_token_env:
        ...
        if not token:
            raise ValueError(
                f"missing inference token environment variable {args.inference_token_env!r}"
            )
    return LightNav0Client(
        VvlaHttpClient(
            args.inference_url, token=token, timeout_s=args.inference_timeout_s
        )
    )
