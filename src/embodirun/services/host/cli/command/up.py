...
from ...source import active_deploy_project, supervisor_path
from ...state import (
    DeploymentState,
    EnvironmentState,
    NodeState,
    ...
    with context.executor(node_id) as executor:
        ...
        for service in services:
            ...
            try:
                ...
                }.get(posixpath.basename(executable))
                if available.exit_code == 1 and legacy_name is not None:
                    # Environments pinned to a pre-rename commit only have the
                    # legacy entry point. Prefer the new name whenever present.
                    executable = posixpath.join(
                        posixpath.dirname(executable), legacy_name
                    )
                    available = executor.run(
                        Command(("test", "-x", executable)), check=False
                    )
                    materialized = replace(
                        materialized,
                        command=replace(
                            materialized.command,
                            argv=(executable, *materialized.command.argv[1:]),
                        ),
                    )
                ...
                supervisor = ServiceSupervisor(
                    executor,
                    python=node.python,
                    agent_path=supervisor_path(executor, deploy_project),
