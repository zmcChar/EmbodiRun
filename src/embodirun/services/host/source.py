...
def _repository_identity(repository: str) -> str:
    """Recognize only the two approved GitHub repository renames.

    Existing managed checkouts keep their origin and directory. Unrelated
    repositories still require an exact URL match.
    """
    ...
    normalized = repository.lower().removesuffix(".git")
    for prefix in (
        ...
        if normalized.startswith(prefix):
            ...
            if name in aliases:
                return f"https://github.com/buaa-ci-lab/{aliases[name]}"
    return repository
...
class ProjectManager:
    ...
    def submodule_revision(
        ...
        *,
        ...
        # Historical deployment commits keep the original gitlink path. Only
        # fall back when the new path is absent, never when it is malformed.
        if not result.stdout.strip() and legacy_path is not None:
            return self.submodule_revision(
                project_dir=project_dir, revision=revision, path=legacy_path
            )
...
def supervisor_path(executor: Executor, project_dir: str) -> str:
    """Find the supervisor in a current or historically pinned source tree."""

    for package in ("embodirun", "rlinf_deploy"):
        path = posixpath.join(project_dir, "src", package, "services/host/supervisor.py")
        result = executor.run(Command(("test", "-f", path)), check=False)
        if result.exit_code == 0:
            return path
        if result.exit_code != 1:
            raise SourceError(f"could not inspect supervisor {path!r}: {result.stderr}")
    raise SourceError(f"no deployment supervisor found in {project_dir!r}")
...
def build_source_archive(value: Path) -> SourceArchive:
    ...
    legacy_module = source / "src/rlinf_deploy.py"
    if legacy_module.is_file():
        files["src/rlinf_deploy.py"] = legacy_module.read_bytes()
