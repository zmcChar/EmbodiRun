"""Two SO-101 followers with separate serial ports and calibration identities."""
...
class BiSO101Config:
    ...
    def __post_init__(self) -> None:
        ...
        if (self.left.calibration_dir, self.left.calibration_id or self.left.robot_id) == (
            self.right.calibration_dir, self.right.calibration_id or self.right.robot_id
        ):
    ...
    def from_mapping(cls, robot_id: str, value: Mapping[str, Any]) -> BiSO101Config:
        ...
        return cls(
            ...
            left=SO101Config.from_mapping(
                f"{robot_id}_left", {**options, "port": left_port, "calibration_id": left_id}
            ),
            right=SO101Config.from_mapping(
                f"{robot_id}_right", {**options, "port": right_port, "calibration_id": right_id}
            ),
        )
