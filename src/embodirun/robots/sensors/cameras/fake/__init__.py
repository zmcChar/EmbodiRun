    ...
    for item in inputs:
        ...
        if unknown:
            raise FakeCameraError(
                f"sensor {item.sensor_id!r} contains unknown fake camera fields: "
                f"{', '.join(unknown)}"
            )
        ...
        try:
            cameras.append(
                FakeCameraConfig(name=item.name, color=color, **{
                    key: value for key, value in options.items() if key != "color"
                })
            )
        except (TypeError, ValueError) as error:
            raise FakeCameraError(
                f"sensor {item.sensor_id!r} has invalid fake camera configuration: "
                f"{error}"
            ) from error
