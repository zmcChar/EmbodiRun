...
class FakeJointsConfig:
    ...
    def from_mapping(
        ...
        if unknown:
            raise ValueError(
                "unknown fake joints configuration fields: " + ", ".join(unknown)
            )
        ...
        if len(joints) != len(FAKE_JOINTS):
            raise ValueError(
                "initial_joint_positions_deg must contain "
                f"{len(FAKE_JOINTS)} values"
            )
        return cls(
            robot_id=robot_id,
            initial_joint_positions_deg=tuple(
                _finite_number(item, f"initial_joint_positions_deg[{index}]")
            ...
            max_joint_step_deg=_finite_number(
                options.get("max_joint_step_deg", 12.0), "max_joint_step_deg"
            ),
            max_gripper_step=_finite_number(
                options.get("max_gripper_step", 20.0), "max_gripper_step"
            ),
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.initial_joint_positions_deg) != len(FAKE_JOINTS):
            raise ValueError(
                f"initial_joint_positions_deg must contain {len(FAKE_JOINTS)} values"
            )
        for index, value in enumerate(self.initial_joint_positions_deg):
            ...
            if not -180.0 <= number <= 180.0:
                raise ValueError(
                    f"initial_joint_positions_deg[{index}] must be in [-180, 180]"
                )
