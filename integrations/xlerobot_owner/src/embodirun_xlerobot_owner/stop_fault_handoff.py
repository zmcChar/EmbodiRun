...
def legacy_safety(robot, snapshot):
    ...
    if (assertion.get("no_enable_attempts_since") is not True
            or arm.get("armed") is not True or arm.get("errors") != []
            or not arm.get("writes") or any(w.get("status") != "accepted" for w in arm["writes"])
            or type(started) is not int or type(completed) is not int
            or type(stop_stamp) is not int or not 0 < started <= completed <= stop_stamp):
    ...
    for key in ("status_before", "status_after"):
        ...
        if (held.get("restored") is not True or held.get("register_writes") != 0
                or held.get("stop_confirmed") is not True or held.get("errors") != []
                or set(held.get("motors", [])) != positions or len(held.get("motors", [])) != len(positions)
    ...
    goals = {w["motor"]: w["value"] for w in snapshot["failed_stop"]["writes"]
             if w["field"] == "Goal_Position"}
    return {"stop_unconfirmed": True, "torque_ownership_uncertain": False,
            "owned_motors": sorted(names), "held_goals": goals,
...
def validate_snapshot(robot, snapshot):
    ...
    if (type(stamp) not in (int, float) or not math.isfinite(stamp)
            or not 0 <= time.monotonic() - stamp <= 120):
    ...
    if (stop.get("scope") != "all" or stop.get("stop_confirmed") is not False
            or stop.get("command_accepted") is not True or stop.get("errors") != []
    ...
    for write in writes:
        ...
        if (write.get("status") != "accepted" or write.get("field") != field
    ...
    for key in ("status_before", "status_after"):
        ...
        if (status.get("connected") is not True or status.get("armed") is not False
                ...
                or status.get("recording") is not False or status.get("error")
                or status.get("observation_errors") or status.get("feedback") != stop):
    ...
    for sample in samples:
        ...
        if (type(stamp) is not int or stamp <= previous or sample.get("state_cached") is not False
                or sample.get("errors") != [] or set(raw) != names
                or any(type(v.get(k)) is not int for v in raw.values()
    ...
    if before is None and after is None:
        ...
        state = before
    if (state.get("stop_unconfirmed") is not True
            or type(state.get("torque_ownership_uncertain")) is not bool
            or not isinstance(state.get("owned_motors"), list)
            or len(state["owned_motors"]) != len(names) or set(state["owned_motors"]) != names
