...
def record_joint_range(args: argparse.Namespace, inspector: Any, *, stop_event: Event | None = None,
                       sleep_fn=None, now_fn=time.monotonic, ready_callback=None, sample_callback=None) -> tuple[dict[str, Any], int]:
    ...
    try:
        ...
        report = {"status": "failed", "final_report": "observed_range_only", "error": f"calibration_read_failed:{type(exc).__name__}: {exc}", "samples": []}
        _write(args.output_dir / "joint_range.json", report)
        return report, 1
    if not isinstance(saved, dict):
        report = {"status": "failed", "final_report": "observed_range_only", "error": "calibration_json_must_be_object", "samples": []}
        _write(args.output_dir / "joint_range.json", report)
        return report, 1
    calibration = saved.get(args.joint)
    if calibration is not None and not isinstance(calibration, dict):
        report = {"status": "failed", "final_report": "observed_range_only", "error": "calibration_joint_entry_must_be_object", "samples": []}
        _write(args.output_dir / "joint_range.json", report)
        return report, 1
    if not (args.joint.startswith("left_arm_") or args.joint.startswith("right_arm_")):
        calibration = None
    report: dict[str, Any] = {
        "status": "failed", "final_report": "observed_range_only", "joint": args.joint,
        "source": "physical_manual_joint_observation", "action": None,
        "calibration_input": {"provided": bool(args.calibration), "path": str(args.calibration) if args.calibration else None},
        "old_range": {"range_min": calibration.get("range_min"), "range_max": calibration.get("range_max")} if calibration else None,
        "samples": [], "observed_min": None, "observed_max": None, "error": None,
        "source_context": {"port": args.port, "sdk_src": getattr(args, "sdk_src", None), "joint": args.joint},
        "range_usable": False, "encoder_wrap_or_discontinuity": False,
    ...
    try:
        bus = inspector.FeetechMotorsBus(
            port=args.port,
            motors={args.joint: inspector.Motor(int(calibration["id"]), "sts3215", inspector.MotorNormMode.RANGE_M100_100)},
        )
        ...
        if model_number != 777:
            ...
            preflight = {}
            for field in ("Homing_Offset", "Min_Position_Limit", "Max_Position_Limit", "Operating_Mode", "Torque_Enable"):
                preflight[field] = bus.read(field, args.joint, normalize=False)
            report["preflight"] = {"actual": preflight, "expected": {
                "Homing_Offset": calibration.get("homing_offset"), "Min_Position_Limit": calibration.get("range_min"),
                "Max_Position_Limit": calibration.get("range_max"), "Operating_Mode": 0, "Torque_Enable": 0}}
            if any(preflight[field] != report["preflight"]["expected"][field] for field in ("Homing_Offset", "Min_Position_Limit", "Max_Position_Limit")):
                ...
                while not stop_event.is_set() and now_fn() - started_mono < args.duration:
                    ...
                    sample: dict[str, Any] = {"source_host_time": time.time(), "monotonic": sample_started, "fields": {}}
                    try:
                        ...
                        if report["samples"] and abs(position - report["samples"][-1]["fields"].get("Present_Position", position)) > 2048:
                            report["encoder_wrap_or_discontinuity"] = True
                            report["range_usable"] = False
                        report["observed_min"] = position if report["observed_min"] is None else min(report["observed_min"], position)
                        report["observed_max"] = position if report["observed_max"] is None else max(report["observed_max"], position)
    ...
    _write(args.output_dir / "joint_range.json", report)
    status = 0 if report["status"] in {"complete", "stopped_partial"} and report["samples"] and not report.get("error") else 1
