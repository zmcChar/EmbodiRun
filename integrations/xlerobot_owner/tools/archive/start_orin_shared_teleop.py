...
def capture_idle(config, token_file, output):
    ...
    client = RemoteRobot("http://127.0.0.1:8766", token_file.read_text().strip(),
                         scope="base", timeout=3)
    before = client._request("status")
    samples = []
    for _ in range(3):
        observation, _ = client.read()
        samples.append({key: observation.get(key) for key in
                        ("state_timestamp_ns", "state_cached", "errors", "raw")})
        time.sleep(.06)
    evidence = {"source": "physical", "boot_id": robot._boot_id(),
                "created_monotonic_s": time.monotonic(), "status_before": before,
                "status_after": client._request("status"), "samples": samples}
...
def capture_stop_fault(config, token_file, output, legacy_arm_run=None, no_enable_since=False):
    ...
    while len(samples) < 3 and time.monotonic() < deadline:
        ...
        if type(stamp) is int and stamp > previous and observation.get("state_cached") is False:
            samples.append({k: observation.get(k) for k in
                            ("state_timestamp_ns", "state_cached", "errors", "raw")})
            previous = stamp
        time.sleep(.06)
    after = client._request("status")
    snapshot = {"source": "physical", "boot_id": robot._boot_id(), "ports": config["ports"],
                "created_monotonic_s": time.monotonic(), "status_before": before,
                "status_after": after, "samples": samples, "failed_stop": after.get("feedback")}
    if legacy_arm_run is not None:
        snapshot["legacy_stop_only_review"] = {
            "last_arm_run": legacy_arm_run, "no_enable_attempts_since": no_enable_since}
...
def prepare(config, output, prior_idle=None):
    ...
    try:
        ...
        robot.connect()
        records = {name: record for bus in robot._last_audit["buses"].values()
                   for name, record in bus.items()}
        ...
        for name in positions:
            ...
            if fields.get("Torque_Enable") == 1:
                ...
                cold.append(name)
        wheel_torque = ({n: records[n]["fields"]["Torque_Enable"] for n in WHEEL_NAMES}
                        if prior_idle is not None else None)
        if wheel_torque is not None and any(type(t) is not int or t not in (0, 1)
                                            for t in wheel_torque.values()):
        ...
        for name in WHEEL_NAMES:
            ...
            if ((prior_idle is None and fields.get("Torque_Enable") != 0)
                    or fields.get("Goal_Velocity") != 0):
        ...
        snapshot = {"source": "physical", "stop_confirmed": True,
                    "created_monotonic_s": time.monotonic(), "ports": config["ports"],
                    "enable_base": False, "goals": goals, "cold_motors": cold,
                    "stationary_wait": {n: r["stationary_wait"] for n, r in audited.items()
        ...
        with os.fdopen(fd, "w") as handle:
            json.dump(snapshot, handle, indent=2)
        print(json.dumps({"handoff": name, "held_goals": goals, "cold_motors": cold,
                          "register_writes": 0}), flush=True)
        return name
    finally:
        robot.close()
...
def main():
    ...
    mode.add_argument("--capture-idle", action="store_true",
                      help="save strict idle evidence via existing API, then exit")
    mode.add_argument("--capture-stop-fault", action="store_true",
                      help="save unchanged failed all-stop evidence via API, then exit")
    mode.add_argument("--resume-stop-fault", type=Path,
                      help="consume fresh fault evidence; start blocked, no stop/arm/torque writes")
    parser.add_argument("--legacy-arm-run", type=Path,
                        help="old API only: reviewed result.json containing latest successful arm")
    parser.add_argument("--confirm-no-enable-since", action="store_true",
                        help="operator confirms no enable ATTEMPTS since that run, in this old service")
    parser.add_argument("--prior-idle", type=Path,
                        help="fresh pre-shutdown idle evidence; permits retaining zero-speed wheel torque")
    args = parser.parse_args()
    if ((args.legacy_arm_run or args.confirm_no_enable_since) and not args.capture_stop_fault
    ...
    if args.capture_stop_fault:
        capture_stop_fault(config, args.token_file, args.output,
    ...
    if args.resume_stop_fault:
        ...
        platform = Platform(HardwareRobot(config), args.token_file.read_text().strip(),
                            output=args.output / "episodes", robot_api=True)
    ...
    os.execv(sys.executable, [sys.executable, "-u", "-m", "embodirun_xlerobot_owner", "serve",
                             "--mode", "hardware", "--robot-api", "--host", "127.0.0.1",
                             "--port", "8766", "--hardware-config", str(args.config),
                             "--resume-held-state", snapshot, "--token-file", str(args.token_file),
                             "--output", str(args.output / "episodes")])
